...
public class LoginControllerTest {
    ...
    @Test
    public void testShouldReturnUnautherizedIfAuthServiceReturnsFalse() {
        LoginController loginController = new LoginController();

        AuthService authService = Mockito.mock(AuthService.class);
        loginController.setAuthService(authService);

        Mockito.when(authService.isValid(Mockito.anyString(), Mockito.anyString())).thenReturn(false);

        UserDTO loginRequestDTO = new UserDTO();
        loginRequestDTO.setUser("maarten");
        loginRequestDTO.setPassword("password");

        Assertions.assertEquals(401, loginController.login(loginRequestDTO).getStatus());
    }
}
